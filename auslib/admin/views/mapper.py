...
from auslib.admin.views.permissions import UsersView
...
def operator_validator(field_value):
    ...
    # print 'starting in operator_validator: buildID is %s' % field_value
...
def version_validator(field_value):
    ...
    # print 'starting in version_validator: version data is %s' % field_value
    if not isinstance(field_value, str_types):
        return True
    # empty input
    if field_value is None or field_value == '':
        return True
    rulesVersionList = field_value.split(",")
